...
#model = lda.build_lda(build_corpus= True, ntop = 10)
...
terms_per_topic = []
for prob, topic in model.show_topics(num_topics = topic_size, formatted = False, num_words = 100):
	words = []
	for word, wprob in topic:
		if (brown_fdist.get(word, 0) < 10 and reuters_fdist.get(word, 0) < 10):
			words.append(word)
	terms_per_topic.append(words)
...
#print(tabulate(terms_per_topic, tablefmt='fancy_grid'))
...
# 	idx += 1
# 	doc_phrases = {}
# 	for topic, topic_dist in model[doc]:
# 		for word in terms_per_topic[topic]:
#pprint(output)
#for name, phrases_in_doc in output.items():
#	topics_per_document[name]['automatic_labeling'] = phrases_in_doc


# with open('output.csv', 'w') as csv_file:
#     wr = csv.writer(csv_file, delimiter=',')
#     for doc_file_name, data in topics_per_document.items():
#     	wr.writerow([doc_file_name, ' '.join(data['word2vec_phrases']), ' '.join(data['automatic_labeling'])])
# output = {}
# idx = 0
# for doc in my_corpus:
# 	doc_name = file_names[idx]
